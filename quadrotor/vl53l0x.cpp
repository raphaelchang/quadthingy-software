...
#define startTimeout() (timeout_start = chVTGetSystemTime())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && chVTTimeElapsedSinceX(timeout_start) > io_timeout)
...
VL53L0X::VL53L0X(bool io_2v8)
    : address(ADDRESS_DEFAULT)
    , io_timeout(MS2ST(500)) // no timeout
      , did_timeout(false)
{
    init(io_2v8);
}
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
    ...
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 2, rxbuf, 0, tmo);
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
    ...
    uint8_t rxbuf[1];
    txbuf[0] = reg;
    txbuf[1] = ((value >> 8) & 0xFF); // value high byte
    txbuf[2] = value & 0xFF; // value low byte
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 3, rxbuf, 0, tmo);
    i2cReleaseBus(&I2C_DEV);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
    ...
    txbuf[4] = value & 0xFF;
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 5, rxbuf, 0, tmo);
    i2cReleaseBus(&I2C_DEV);
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
    ...
    txbuf[0] = reg;
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 1, rxbuf, 1, tmo);
    i2cReleaseBus(&I2C_DEV);
    return rxbuf[0];
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    ...
    txbuf[0] = reg;
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 1, rxbuf, 2, tmo);
    i2cReleaseBus(&I2C_DEV);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    txbuf[0] = reg;
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 1, rxbuf, 4, tmo);
    i2cReleaseBus(&I2C_DEV);
    return ((uint32_t)rxbuf[0] << 24) | ((uint32_t)rxbuf[1] << 16) | ((uint16_t)rxbuf[2] << 8) | rxbuf[3];
}
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
    ...
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, count + 1, rxbuf, 0, tmo);
    i2cReleaseBus(&I2C_DEV);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
    ...
    txbuf[0] = reg;
    i2cAcquireBus(&I2C_DEV);
    last_status = i2cMasterTransmitTimeout(&I2C_DEV, address, txbuf, 1, dst, count, tmo);
    i2cReleaseBus(&I2C_DEV);
}

...
Controller::Controller()
    : m_vel_p_gains(0.02, 0.02, 0.001),
    m_vel_i_gains(0, 0, 0),
    m_vel_d_gains(0, 0, 0),
    m_vel_ff_gains(0.05, 0.05, 0.01),
    m_pos_p_gains(0.005, 0.005, 0.01),
    m_pos_i_gains(0, 0, 0),
    m_pos_d_gains(0, 0, 0),
    m_orientation(0, 0, 0),
    m_rotation(0, 0, 0)
...
void Controller::velocityLoopUpdate(Vector3d setpoint)
{
    ...
    m_md1->Set(sqrt(fabs(CLAMP_ZERO(m_throttle - pitchOut + rollOut + yawOut))));
    m_md2->Set(-sqrt(fabs(CLAMP_ZERO(m_throttle - pitchOut - rollOut - yawOut))));
    m_md3->Set(sqrt(fabs(CLAMP_ZERO(m_throttle + pitchOut - rollOut + yawOut))));
    m_md4->Set(-sqrt(fabs(CLAMP_ZERO(m_throttle + pitchOut + rollOut - yawOut))));
}
